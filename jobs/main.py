...
# Environment Variables for confirguration
KAFKA_BOOTSTRAP_SERVERS = os.getenv('KAFKA_BOOTSTRAP_SERVERS', 'localhost:9092')
VEHICLE_TOPIC = os.getenv('VEHICLE_TOPIC', 'vehicle_date')
...
def delivery_report(err, msg):
    try:
        ...
        print(f"Error in delivery report: {e}")
...
def produce_data_to_kafka(producer, topic, data):
    print(f"Producing to {topic}: {data}")  # Debug statement
    producer.produce(
        topic,
        key=str(data['id']),
...
#simulate journey is going to having all info about vehicle and at the end of the day we're going to producing data from here
def simulate_journey(producer, device_id):
    while True:
        ...
        if (vehicle_data['location'][0] >= BIRMINGHAM_COORDINATES['latitude']
            ...
            print('Vehicle has reached Birmingham. Simulating ending...')
            break
...
if __name__ == "__main__":
    producer_config = {
        'bootstrap.servers': KAFKA_BOOTSTRAP_SERVERS,  # Corrected key
